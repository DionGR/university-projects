...
class ImprovedModel(torch.nn.Module):
    """
    This is an improved model for SSD.
    ...
    def __init__(self,
                 ...
                 image_channels: int,
                 output_feature_sizes: List[Tuple[int, int]]):
        super().__init__()
        ...
        self.fmap_1 = torch.nn.Sequential(
            ...
            torch.nn.BatchNorm2d(num_filters_conv),
            torch.nn.PReLU(),
            ...
            torch.nn.Conv2d(in_channels=num_filters_conv,
                            ...
                            kernel_size=filter_size_conv,
                            stride=stride_conv,
                            padding=padding_conv * 2),
            torch.nn.BatchNorm2d(num_filters_conv * 2),
            ...
            torch.nn.Conv2d(in_channels=num_filters_conv * 2,
                            ...
                            padding=padding_conv),
            torch.nn.BatchNorm2d(self.out_channels[0]),
            torch.nn.PReLU(),
        )
        ...
        self.fmap_2 = torch.nn.Sequential(
            ...
            torch.nn.Conv2d(in_channels=self.out_channels[0],
                            ...
                            stride=stride_conv,
                            padding=padding_conv),
            torch.nn.BatchNorm2d(num_filters_conv * 4),
            torch.nn.PReLU(),
            ...
            torch.nn.Conv2d(in_channels=num_filters_conv * 4,
                            ...
                            stride=stride_conv * 2,
                            padding=padding_conv),
            torch.nn.BatchNorm2d(self.out_channels[1]),
            torch.nn.PReLU(),
        )

        self.fmap_3 = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=self.out_channels[1],
                            out_channels=num_filters_conv * 8,
                            kernel_size=filter_size_conv,
                            stride=stride_conv,
                            padding=padding_conv),
            torch.nn.BatchNorm2d(num_filters_conv * 8),
            torch.nn.PReLU(),

            torch.nn.Conv2d(in_channels=num_filters_conv * 8,
                            out_channels=self.out_channels[2],
                            kernel_size=filter_size_conv,
                            stride=stride_conv * 2,
                            padding=padding_conv),
            torch.nn.BatchNorm2d(self.out_channels[2]),
            torch.nn.PReLU(),
        )
        ...
        self.fmap_6 = torch.nn.Sequential(
            ...
            torch.nn.Conv2d(in_channels=num_filters_conv * 2,
                            ...
                            stride=stride_conv * 2,
                            padding=padding_conv), 
            torch.nn.BatchNorm2d(self.out_channels[5]),
            torch.nn.PReLU(),
        )

        self.fmap_7 = torch.nn.Sequential(
            ...
            torch.nn.Conv2d(in_channels=num_filters_conv * 2,
                            ...
                            stride=stride_conv,
                            padding=0), 
            #torch.nn.BatchNorm2d(self.out_channels[6]),
            torch.nn.PReLU(),
        )
