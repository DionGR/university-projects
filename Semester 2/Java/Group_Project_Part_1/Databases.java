...
public class Databases {
    ...
    */
    ...
    public int getEmployeeByCode(String code) {
        ArrayList<Employee> employees = this.employees;
        for (int i = 0; i < employees.size(); i++) {
            if (employees.get(i).getCode().equals(code)) return i;
        }

        return -1;
    }

    public int getExpenseByCode(String code) {
        ArrayList<ExpenseType> expenses = this.expenseTypes;
        for (int i = 0; i<expenses.size(); i++) {
            if (expenses.get(i).getCode().equals(code)) return i;
        }

        return -1;
    }

    public ArrayList<ExpenseType> getExpenseTypes() { return this.expenseTypes; }
}
